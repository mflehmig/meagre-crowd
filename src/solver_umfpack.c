...
typedef struct
{
  ...
  void* Symbolic;
  void* Numeric;
  double Control[UMFPACK_CONTROL];  //X1
...
void solver_analyze_umfpack(solver_state_t* s, matrix_t* A)
{
  ...
  if (s->verbosity >= 4) {
    //X1 double Control[UMFPACK_CONTROL];
    //X1 Control --> p->Control
  ...
  int status = umfpack_di_symbolic(A->m, A->n, (int*) A->jj, (int*) A->ii, A->dd, &(p->Symbolic), p->Control/*X1 NULL*/, NULL);
...
void solver_factorize_umfpack(solver_state_t* s, matrix_t* A)
{
  ...
  int status = umfpack_di_numeric(p->Ajj, p->Aii, p->Add, p->Symbolic, &(p->Numeric), p->Control/*X1 NULL*/, NULL);
  if (status != UMFPACK_OK)
    umfpack_di_report_status(NULL, status);
  assert(status == UMFPACK_OK);
}
